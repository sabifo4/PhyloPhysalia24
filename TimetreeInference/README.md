# Bayesian timetree inference with `PAML` programs
...
We will run 6 chains when sampling from the prior and 6 chains when sampling from the posterior under the independent-rates log-normal (ILN) relaxed-clock model and the geometric Browniand motion model (GBM, also known as the autocorrelated-rates model):
...
We can now open `FigTree` and plot the recently created `node_tree.tree` alongside the calibrated tree file `mtCDNApri.trees`:
...
### (EXTRA) Additional MCMC diagnostics

#### Prior (extra)

We are going to run the R script [`MCMC_diagnostics_prior.R`](scripts/MCMC_diagnostics_prior_terminal.R) and follow the detailed step-by-step instructions detailed in the script. In a nutshell, the protocol will be the following:
...
<i>Unfortunately, we will not have time to go through these scripts but, when you finish the tutorial, feel free to open RStudio and run [the in-house R script](00_inp_data/calibs/Include_calibrations_MCMCtree.R).

If you have run `MCMCtree` and generated output file [`node_tree.tree`](02_PAML/01_MCMCtree/00_prior/node_tree.tree), you can then run the second [in-house R script `Merge_node_labels.R`](scripts/Merge_node_labels.R) in RStudio, which will output the [`Calibnodes_mtcdnapri.csv` file](00_inp_data/calibs/inp_calibs/Calibnodes_mtcdnapri.csv) that you will need for the subsequent steps of the MCMC diagnostics!</i>

</details>
<br>

We will run our in-house R script [`MCMC_diagnostics_prior_terminal.R`](03_ExtraMCMCdiagnostics/scripts/MCMC_diagnostics_prior_terminal.R) to carry out these additional MCMC diagnostics. This script only needs one argument: the absolute path to the `scripts` directory (i.e., type `pwd` on a Terminal open in this location to know its absolute path). Once you know the absolute path to such directory, please do the following:
...
Now, once the MCMC diagnostics have finished, you can extract the final data that you can use to summarise your results in an easy way without having to navigate the file structure!:

```sh
## Run from `03_ExtraMCMCdiagnostics`
# If you are not in this directory, please change
# until you are there
mkdir sum_files_prior
cp -R ../02_PAML/01_MCMCtree/00_prior/mcmc_files*NODAT/*NODAT*tree sum_files_prior/
cp -R ../02_PAML/01_MCMCtree/00_prior/NODAT/*/*all_mean*tsv sum_files_prior/
...
We will run the R script [`MCMC_diagnostics_posterior_template.R`](03_ExtraMCMCdiagnostics/scripts/MCMC_diagnostics_posterior_template.R) and follow the detailed step-by-step instructions detailed in the script, which are essentially the same ones you used when analysing the samples collected when sampling from the prior.

```sh
## Run from `03_ExtraMCMCdiagnostics/scripts` directory
# Change directories until you are there
#
# Arg1  Absolute path to directory `03_ExtraMCMCdiagnostics/scripts`
scripts_dir=$( pwd )
Rscript MCMC_diagnostics_posterior_terminal.R $scripts_dir
```

Once you have your final timetrees estimated under the two different relaxed-clock models (yay!), we can run our [in-house R script](03_ExtraMCMCdiagnostics/scripts/Check_priors_VS_posteriors_terminal.R) to compare various distributions: marginal densities, calibration densities, and posterior time densities! These plots can help assess how informative the data are and check whether there are serious differences between the marginal densities and the posterior time densities.
...
# Arg1  Absolute path to directory `03_ExtraMCMCdiagnostics/scripts`
scripts_dir=$( pwd )
Rscript Check_priors_VS_posteriors_terminal.R $scripts_dir
```
...
cp -R ../02_PAML/01_MCMCtree/01_posterior/mcmc_files_*/FigTree*tree sum_files_post/
cp -R ../02_PAML/01_MCMCtree/01_posterior/*/*/*all_mean*tsv sum_files_post/
