...
# Function that is used when the densities of equality and inequality
# constraints, including ghost branches, are to be plotted.
#
# Arguments:
#
# dup_dat      List, as many elements as nodes have been constrained using
#              inequality and equality constraints.
#              IMPORTANT: the first elements have to be equality constraints
#              (i.e. cross-bracing; labels starts with "CB-FC-[A-Z]*"),
#              followed by inequality constraints (both recipient and donor
#              nodes; labels start with "CB-1(D|R)-[A-Z]*"). The last element
#              includes all the nodes (if there is more than one) that have
#              been used as ghost branches (label could be
#              "all-gbr-flag[0-9]")
# mcmc_divt    Data frame, object present in the list `mcmc_priors` created
#              in script `Check_priors_margVScalib.R`. Access the data frame
#              as mcmc_priors`<name_hytpohresis>$divt.
# out_dirname  Character, name of the hypothesis. Must be same name as in
#              `<name_hypothesis` in object `mcmc_divt`.
# pos_ineq     Integer, minimum and maximum number identifying the range of the
#              positions in object `dup_dat` where you have equality and
#              inequality constraints. E.g., `c(21,24)`. Ghost branches are not
#              to be included (i.e., do not include the integer corresponding
#              to the last element in object `dup_dat`).
# ghost_nodes  Integer, position of the ghost branches in `dup_dat`; the last
#              element. E.g., `25`.
# donor_name   Character, name given to the donor label.
#              E.g., "CB-1D-RHODOPHYTA"
# cols_vec     Character, list of colours. Default: c( "blue", "red", "purple",
#              "darkgreen", "brown" ). At the moment, only the first colour
#              is used to draw the marginal densities.
# calib_mat    Data frame, object created in R script
#              `Check_priors_margVScalib.R`. First element has the name of the
#              label, second column the node that is constrained, and third
#              column the MCMCtree notation to identify such a constraint.
plot_dupnodes <- function( dup_dat, mcmc_divt, out_dirname, pos_ineq,
                           ghost_nodes, donor_name, 
                           cols_vec = c( "blue", "red", "purple",
                                         "darkgreen", "brown" ),
                           calib_mat )
{
  
  # Define global vars
  name_labs <- colnames( mcmc_divt )
  count <- 0
  if( ! dir.exists( paste( home_dir, "plots/dupnodes/", sep = "" ) ) ){
    dir.create( paste( home_dir, "plots/dupnodes/", sep = "" ) )
  }
  if( ! dir.exists( paste( home_dir, "plots/dupnodes/", out_dirname, "/",
                           sep = "" ) ) ){
    dir.create( paste( home_dir, "plots/dupnodes/", out_dirname, "/",
                       sep = "" ) )
  }
  for( i in 1:length( dup_dat ) ){
    pdf( file = paste( home_dir, "plots/dupnodes/", out_dirname, "/dupnodes_",
                       out_dirname, "_", names(dup_dat)[i], "_plot.pdf",
                       sep = "" ), 
         paper = "a4r", width = 0, height = 0 )
    ...
    if( tot_nodes == 2 ){
      ...
      par( mfrow = c(3,2) )
    }else if( tot_nodes == 7 | tot_nodes == 8 ){
      par( mfrow = c(3,3) )
    }
    # If there are inequality constraints...
    # --> Range of pos for ineq. constraints goes from `pos_ineq[1]`
    # --> to `pos_ineq[2]`
    # --> Here, we plot recipient nodes VS the donor node
    if( i > c(pos_ineq[1]-1) && i < c(pos_ineq[2]+1) ){
      # Set max and min -- to be tailored for other plots, these settings
      # fit current dataset!
      cat( "\n[[ Constraint ", i, " is an inequality constraint | ",
           "There are ", tot_nodes, "nodes ]]\n" )
      # Find donor node
      pos_D_donors <- which( names(dup_dat) == donor_name )
      node_D <- which( name_labs %in% dup_dat[[ pos_D_donors ]] )
      # Find max/min
      #max_x <- 1.6
      #min_x <- 0.0
      max_x <- max( density( mcmc_divt[[ nod_ind[j] ]], adj = 1 )$x,
                    density( mcmc_divt[[ node_D[1] ]], adj = 1 )$x   ) + 0.2
      min_x <- min( density( mcmc_divt[[ nod_ind[j] ]], adj = 1 )$x,
                    density( mcmc_divt[[ node_D[1] ]], adj = 1 )$x ) - 0.2
      max_y <- max( density( mcmc_divt[[ nod_ind[j] ]], adj = 1 )$y,
                    density( mcmc_divt[[ node_D[1] ]], adj = 1 )$y ) + 0.2
      # Iterate over all "j" nodes that are constrained with "i"-th calibration
      # Plot one density per node VS the recipient node
      for( j in 1:tot_nodes ){
        # Plot density for the first node that has this constraint...
        cat( "---> Plot node number ", j, "(", dup_dat[[ i ]][j],
             ") VS donor node \n" )
        plot( density( mcmc_divt[[ nod_ind[j] ]], adj = 1 ),
              xlim = c(min_x,max_x),
              ylim = c(0,max_y),
              main = paste( "[",
                            names(dup_dat)[i], " - ",
                            dup_dat[[ i ]][j], "] VS Donor",
                            sep = "" ),
              cex = 0.6 )
        # Add donor node!
        # IF there are more than 2 donor nodes...
        if( length( node_D ) > 1 ){
          test_eq_D <- all.equal( mcmc_divt[[ node_D[1] ]],
                                  mcmc_divt[[ node_D[2] ]] )
          if( test_eq_D == TRUE ){
            lines( density( mcmc_divt[[ node_D[1] ]],
                            adj = 1 ), col = "blue", type = "s" )
            # Add legend for donor node/s
            text( x = mean( mcmc_divt[[ node_D[1] ]] ),
                  y = 0.1,
                  paste( "mean_t = ", round( mean( mcmc_divt[[ node_D[1] ]] ),
                                             4 ),
                         " | ", donor_name, " | Node/s",
                         paste( dup_dat[[ pos_D_donors ]], collapse = ", " ),
                         sep = "" ),
                  col = "blue",
                  cex = 0.6 )
            # Add legend for recipient node
            text( x = mean( mcmc_divt[[ nod_ind[1] ]] ),
                  y = 0.4,
                  paste( "mean_t = ", round( mean( mcmc_divt[[ nod_ind[1] ]] ),
                                             4 ), " | ",
                         names(dup_dat)[i], " | Node: ", dup_dat[[ i ]][j],
                         sep = "" ),
                  col = "black",
                  cex = 0.6 )
          }else{
            stop( "The two cross-braced donor nodes are not equal!" )
          }
          # IF there is only one donor node...
        }else{
          lines( density( mcmc_divt[[ node_D[1] ]],
                          adj = 1 ), col = "blue" )
          # Add legend for donor node/s
      ...
      # If the node has ghost branch/es, plot them all!
    }else if( i == ghost_nodes ){
      cat( "\n[[ Constraint ", i, " includes ghost branch/es | ",
           "There is/are ", length(dup_dat[[i]]), "node/s ]]\n" )
      for( j in 1:length(dup_dat[[i]]) ){
        # Get donor node
        pos_D_donors <- which( names(dup_dat) == donor_name )
        node_D <- which( name_labs %in% dup_dat[[ pos_D_donors ]] )
        # Plot ghost branch
        ...
        if( length( node_D ) > 1 ){
          ...
          if( test_eq_D == TRUE ){
            ...
            text( x = mean( mcmc_divt[[ node_D[1] ]] ),
                  ...
                  cex = 0.6 )
            # Add legend for recipient node
            text( x = mean( mcmc_divt[[ nod_ind[j] ]] ),
                  y = 0.4,
                  paste( "mean_t = ", round( mean( mcmc_divt[[ nod_ind[j] ]] ),
                                             4 ), " | ",
                         names(dup_dat)[i], " | Node: ", dup_dat[[ i ]][j],
                         sep = "" ),
                  col = "black",
          ...
          lines( density( mcmc_divt[[ node_D[1] ]],
                          adj = 1 ), col = "blue" )
          # Add legend for donor node
          text( x = mean( mcmc_divt[[ node_D[1] ]] ),
                y = 0.1,
                paste( "mean_t = ", round( mean( mcmc_divt[[ node_D[1] ]] ),
                                           4 ),
                       " | ", donor_name, " | Node: ",
                       paste( dup_dat[[ pos_D_donors ]] ),
                       sep = "" ),
                col = "blue",
                cex = 0.6 )
          # Add legend for recipient node
    ...
    # If they are cross-braced nodes (equality constraints)...
    }else{
      # For all "j" nodes constrained under calibration "i"...
      cat( "\n[[ Constraint ", i, " includes equality constraints | ",
           "There are ", tot_nodes, "node/s ]]\n\n" )
      for( j in 1:tot_nodes ){
        # Run the following only for the first node, which is the one that
        # has MCMCtree notation info!
        if( j == 1 ){
          # Set max/min limits for plots
          max_x <- max( density( mcmc_divt[[ nod_ind[j] ]], adj = 1 )$x ) + 0.2
          min_x <- min( density( mcmc_divt[[ nod_ind[j] ]], adj = 1 )$x ) - 0.2
          # Find calibration density
          row_pos <- which( paste( "t_n", calib_mat$node, sep = "" ) == 
                              dup_dat[[ i ]][j] )
          MCMCtree_calib <- calib_mat[row_pos,]
          info_marg_dens <- find_MCMCtree_calib( calibs = MCMCtree_calib ) 
          # Find new max and min after getting info about marg density
          max_x <- max( max_x, info_marg_dens$tmp_mg_x )
          min_x <- min( min_x, info_marg_dens$tmp_mg_x )
          # Plot  density plot for first node that is constrained under this
          # calibration
          cat( "---> Plot first node | ", dup_dat[[ i ]][j], "\n" )
          plot( density( mcmc_divt[[ nod_ind[j] ]], adj = 1 ),
                xlim = c( min_x, max_x ),
          ...
          # Plot density plot for the rest of the nodes that are constrained
        ...
        # Add marginal density too
        if( length( info_marg_dens$type_cal$B ) == 1 | 
            length( info_marg_dens$type_cal$U ) == 1 | 
            length( info_marg_dens$type_cal$L ) == 1 ){
          lines( x = info_marg_dens$tmp_mg_x, y = info_marg_dens$tmp_mg_y,
                 adj = 1 , col = cols_vec[1] )
        }
        if( length( info_marg_dens$type_cal$ST ) == 1 ){
          curve( dst( x, xi = info_marg_dens$ST_vals[1],
                      omega = info_marg_dens$ST_vals[2],
                      alpha = info_marg_dens$ST_vals[3],
                      nu = info_marg_dens$ST_vals[4] ),
                 from = 0, to = c(min_x,max_x),
                 n = 1e4, add = TRUE, col = cols_vec[1] )
        }
        if( length( info_marg_dens$type_cal$SN ) == 1 ){
          curve( dst( x, xi = info_marg_dens$SN_vals[1],
                      omega = info_marg_dens$SN_vals[2],
                      alpha = info_marg_dens$SN_vals[3] ),
                 from = 0, to = c(min_x,max_x),
                 n = 1e4, add = TRUE, col = cols_vec[1] )
        }
      }
    }
    # Lastly, reset the counter for the next iteration for the next node
    count <- 0
    # Close graphics
    dev.off()
  }
...
# Function that identifies the elements to plot MCMCtree distributions.
# At the moment, only soft-bound calibrations (B),
# lower-bound calibrations (L), upper-bound calibrations, ST distributions,
# and SN distributions are implemented. Other functions in this script,
# such as `plot_dupnodes` wrap over this function!
#
# Arguments:
# 
# calibs  Data frame, object that contains three columns and can be generated
#         using function `read_calib_f()` or your own. First column contains
#         node labels, second column the node number as per MCMCtree, and 
#         the third column is the calibration used to constrain that node
#         n MCMCtree notation
#
find_MCMCtree_calib <- function( calibs ){
  
  # Check whether the calibration used was skew-T or soft-bound
  is_ST   <- grep( pattern = "ST", x = calibs[3])
  is_B    <- grep( pattern = "B", x = calibs[3])
  is_L    <- grep( pattern = "L", x = calibs[3])
  is_U    <- grep( pattern = "U", x = calibs[3])
  is_SN   <- grep( pattern = "SN", x = calibs[3])
  type_cals <- list( is_ST, is_B, is_L, is_U, is_SN )
  names( type_cals ) <- c( "ST", "B", "L", "U", "SN" )
  if( length( is_B ) == 1 ){
    cat( paste( "Node ", "t_n", calibs[2], " - ", calibs[1], 
                " is calibrated with a soft-bound: ",
                calibs[3], "\n", sep = "" ) )
    # Clean labels if needed
    clean_lab <- gsub( x = calibs[3], pattern = "-\\[..*", replacement = "" )
    # NOTE: That's a nice wrapper, but users with R < 4.1 do not have
    # `str_split_.
    # I have decided to then use `str_split`
    # tmp_allB <- as.numeric( stringr::str_split_1( string = gsub( pattern = "B\\(|\\)",
    #                                                              replacement = "",
    #                                                              x = calibs[3] ),
    #                                               pattern = "," ) )
    ...
    tmp_x   <- seq( tmp_min-0.5, tmp_max+0.5, 0.001 )
    tmp_y   <- softbounds( t = tmp_x, tL = tmp_min, tU = tmp_max, pL = tmp_pL,
                           pU = tmp_pU )
    tmp_labcab <- substr( x = calibs[1], start = 1, stop = 12 )
  }
  if( length( is_ST ) == 1 ){
    cat( paste( "Node ", "t_n", calibs[2],
                " is calibrated with a skew-t dist: ", " - ", calibs[1], 
                calibs[3], "\n", sep = "" ) )
  ...
  if( length( is_SN ) == 1 ){
    cat( paste( "Node ", "t_n", calibs[2],
                " is calibrated with a skew-normal dist: ", " - ", calibs[1], 
                calibs[3], "\n",  sep = "" ) )
  ...
  if( length( is_L ) == 1 ){
    cat( paste( "Node ", "t_n", calibs[2],
                " is calibrated with a lower bound: ", " - ", calibs[1], 
                calibs[3], "\n",  sep = "" ) )
    ...
    tmp_x   <- seq( tmp_min-1, tmp_min+2, 0.001 )
    tmp_y   <- lowerbounds( t=tmp_x, tL=tmp_min, p = tmp_p, c = tmp_c,
                            pL = tmp_pL )
    tmp_labcab <- substr( x = calibs[1], start = 1, stop = 12 )
  }
  if( length( is_U ) == 1 ){
  ...
  # Return list object
  if( length( is_ST ) == 1 ){
    return( list( tmp_mg_x = tmp_x, tmp_mg_y = tmp_y, tmp_labcab = tmp_labcab,
                  type_cal = type_cals, STvals = tmp_ST_pars ) )
  }else if( length( is_SN ) == 1 ){
    return( list( tmp_mg_x = tmp_x, tmp_mg_y = tmp_y, tmp_labcab = tmp_labcab,
                  type_cal = type_cals, SNvals = tmp_SN_pars ) )
  }else{
    return( list( tmp_mg_x = tmp_x, tmp_mg_y = tmp_y, tmp_labcab = tmp_labcab,
                  type_cal = type_cals ) )
  }
