...
# You can check these numbers in the tsv output file inside `out_RData` that
# were output by scripts `MCMC_diagnostics_prior_terminal.R` and
# `MCMC_diagnostics_posterior_terminal.R`, last row # `total_samples`.
# In this case, 120006 samples for all analyses, and so you need to specify
# one less below.
...
delcol_obj <- c( c( 1, 2, 2 ) )  # dataset1: NODAT, GBM, ILN
# --> If there were more datasets, add the corresponding columns in the
#     vector above
...
# approaches.
num_dirs  <- length( dat )*3 # Keep "3" if three models tested: NODAT, GBM, ILN
...
for( i in dat ){
  count <- count + 1
  names( mcmc_obj )[count] <- paste( "NODAT_", i, sep = "" )
  count <- count + 1
