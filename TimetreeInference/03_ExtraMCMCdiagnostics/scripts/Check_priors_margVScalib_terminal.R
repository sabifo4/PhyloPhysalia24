...
# was output by script `MCMC_diagnostics_prior_terminal.R`, last row
# `total_samples`.
# In this case, `total_samples = 120006`, and so you need to specify
# one less below.
#
...
dat_ff <- list.files( path = "../../00_inp_data/calibs/inp_calibs/",
                      ...
                      full.names = FALSE )
##> CHECK
# If there was a `margVScalib` generated, use it!
...
names( mcmc_priors ) <- paste( "NODAT_", dat, sep = "" )
count <- 0
...
for( i in 1:num_dirs ){
  ...
  ## If you do not want to output plots in a specific format, just comment
  ## the whole block for such format!
